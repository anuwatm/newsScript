...
if ($conn->connect_error) {
	...
	if (isset($_GET['t'])) {$tabID=$_GET["t"];} else {$tabID="";};
	...
	if ($strCir!=""){
        $sql="select * from vscripttable where (tblID=$tabID  $strCir) order by scID desc";
	} else {
		$sql="select * from script where (tblID=$tabID) order by scID desc";
	}
	$result = $conn->query("$sql  LIMIT $itemStart,$recPage");
    //echo("$sql  LIMIT $itemStart,$recPage");
	$row = $result->num_rows;
