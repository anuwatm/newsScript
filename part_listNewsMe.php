...
if ($conn->connect_error) {
	...
	if ($search!=""){
		$sql="select * from script where (title like '%$search%' and modifyBy='" . $_SESSION["usrName"] . "') order by scID desc";
	} else {
		$sql="select * from script where (modifyBy='" . $_SESSION["usrName"] . "') order by scID desc";
	}
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
            ...
            $cssStatus=$process->setMenuMe($row["statusID"],$row["modifyBy"],$_SESSION["usrName"],$_SESSION['groupMe'],$_SESSION['approve'],$_SESSION['otherGroup']);
			if ($search!=""){
				$tableList=$tableList . "<tr><td>$imgStatus</td><td  class='$cssStatus'><span class='status_" . $row["statusID"] . "' style='cursor:hand' onclick=\"javascript:showContextMenu('" . $row["scID"] . "')\">" . str_replace($search,"<span class='keyword'>$search</span>",$row["title"]) . "</span><div class='desc'>" . $row["createDate"] . "</div></td></tr>";
			} else {
